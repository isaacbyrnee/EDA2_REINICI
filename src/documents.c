...
DocumentList *load_documents(char *half_path, int num_docs,
  ...
  while (doc_iter != NULL) {
    ...
    // Crear una llista temporal per registrar els IDs de destí ja afegits per aquest doc_iter
    LinkList *processed_links_for_this_doc = LinksInit(); // Reutilitzem la teva estructura LinkList
    ...
    while (current_link != NULL) {
        ...
        if (graph_node_exists(graph, current_link->id)) {
            ...
            // 2. Només afegim l'aresta si aquest enllaç (doc_iter -> current_link->id) NO HA ESTAT JA PROCESSAT per doc_iter
            
            // Comprovem si aquest linkId ja ha estat processat per l'actual doc_iter
            ...
            if (doc_iter->id != current_link->id && !already_processed_for_this_doc) {
                graph_add_edge(graph, doc_iter->id, current_link->id);
                // Ara que l'hem afegit, registrem que aquest enllaç ja ha estat processat per doc_iter
